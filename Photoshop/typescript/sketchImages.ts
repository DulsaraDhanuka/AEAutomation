...
function sketchLayers(layers: Layers = app.activeDocument.layers): void {
    for (var i = 0; i < layers.length; i++) {
        let layer: Layer = layers[i];

        if (layer.typename == "LayerSet") {
            sketchLayers((<LayerSet>layer).layers);
        } else {
            if (getLayerColourByID(layer.id) == "grain") {
                ...
                let originalLayer = app.activeDocument.layers[0];
                ...
                let sketchGroup: LayerSet = <LayerSet>app.activeDocument.layerSets.add();
                ...
                let prevName: string = app.activeDocument.activeLayer.name;
